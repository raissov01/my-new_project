...
export function SetCard({
  ...
  return (
    <article className="ui-panel ui-interactive flex h-full flex-col p-5 sm:p-6">
      <StudyRoutePrefetch setId={id} />
      <div className="flex items-start justify-between gap-4">
        <div className="min-w-0">
          <Link href={`/sets/${id}`} className="group block">
            <h3 className="line-clamp-1 text-xl font-semibold tracking-[-0.03em] text-[var(--text-primary)] transition-colors group-hover:text-indigo-400">
              {title}
            </h3>
          </Link>
          <p className="mt-3 line-clamp-2 min-h-12 text-sm leading-7 text-[var(--text-secondary)]">
            {description || t("sets.noDescription")}
          </p>
        </div>
        <span className="inline-flex shrink-0 items-center gap-1.5 rounded-full border border-[var(--border)] bg-[var(--bg-surface)] px-3 py-1.5 text-xs font-medium text-[var(--text-secondary)]">
      ...
      <div className="mt-6 grid gap-3 sm:grid-cols-2">
        <div className="ui-subtle px-4 py-4">
