...
import { Users, Target, Timer, Pencil, Play, Lock, Globe } from "lucide-react";
...
function gradientForQuiz(id: string) {
  // Stable per-quiz gradient so resort/refilter doesn't shuffle visuals.
  let hash = 0;
...
export function QuizCard({ quiz, locale, isOwner }: QuizCardProps) {
  ...
  const gradient = gradientForQuiz(quiz.id);

  const visibilityLabel = quiz.isPublic ? t("quiz.isPublic") : t("quiz.isPrivate");

  return (
    <article className="nd-lib-card">
      <Link
        href={quizHref}
      ...
      >
        ...
        <div className="nd-lib-cover" style={{ background: gradient }}>
          ...
          {isOwner ? (
            <div
              style={{ position: "absolute", top: 10, left: 10, background: "rgba(0,0,0,.45)", backdropFilter: "blur(8px)", borderRadius: 99, padding: "3px 8px", fontSize: 10, fontWeight: 700, fontFamily: "'JetBrains Mono',monospace", letterSpacing: ".06em", color: "#fff", display: "flex", alignItems: "center", gap: 4 }}
              aria-label={visibilityLabel}
            >
              {quiz.isPublic ? <Globe style={{ width: 9, height: 9 }} aria-hidden /> : <Lock style={{ width: 9, height: 9 }} aria-hidden />}
              {visibilityLabel}
            </div>
          ) : null}
        </div>
        ...
        <div className="nd-lib-foot">
          <span
            ...
            aria-label={`${t("quiz.attempts")}: ${quiz.attemptsCount}`}
            style={{ display: "flex", alignItems: "center", gap: 4 }}
          >
            <Users style={{ width: 11, height: 11 }} aria-hidden />
            {quiz.attemptsCount}
          </span>
          <span
            title={t("quiz.avgScore")}
            aria-label={`${t("quiz.avgScore")}: ${quiz.averagePercentage}%`}
            style={{ display: "flex", alignItems: "center", gap: 4 }}
          >
          ...
          <span
            title={t("quiz.timePerQuestionLabel")}
            aria-label={`${t("quiz.timePerQuestionLabel")}: ${quiz.timePerQuestion}${t("quiz.secondsShort")}`}
            style={{ display: "flex", alignItems: "center", gap: 4 }}
          >
            <Timer style={{ width: 11, height: 11 }} aria-hidden />
      ...
      {/* Action buttons (outside the wrapping link to avoid nested <a>) */}
      <div style={{ padding: "0 16px 16px", display: "flex", gap: 8, alignItems: "center" }}>
        ...
        >
          <Play style={{ width: 13, height: 13 }} aria-hidden />
          {t("quiz.startQuiz")}
        </Link>
        {isOwner ? (
          <Link
